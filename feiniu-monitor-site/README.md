...
The website list is grouped like folders. Current default groups are business websites, management tools, system entries, and other websites. Users can create, delete, and move sites between folders from the monitor UI; empty folders are shown too. Unsaved folder edits are kept on screen during auto-refresh. The admin password is cached only in browser session storage. Changes are stored in `/vol1/docker/feiniu-monitor-site/data/site-folders.json`.
